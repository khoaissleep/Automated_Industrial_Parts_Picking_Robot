...
# Đường dẫn file cấu hình dữ liệu segmentation
data_yaml = "/home/khoa_is_sleep/screws/AI_training/data_seg.yaml"
...
model = YOLO("yolov8n-seg.pt")  # Sử dụng mô hình segmentation cơ bản

# Huấn luyện mô hình
model.train(
    data=data_yaml,       # File cấu hình dataset segmentation
    epochs=30,            # Số epoch
    batch=2,              # Batch nhỏ vì train trên CPU
    imgsz=640,            # Kích thước ảnh
    device=device,        # Train trên CPU
    task="segment",       # Chỉ định nhiệm vụ là segmentation
    conf=0.001,           # Ngưỡng confidence thấp
...
model.export(format="onnx", filename="screw_segmentation_model") 